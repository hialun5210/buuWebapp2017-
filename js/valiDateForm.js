...
(function(window,doc){
    ...
    validate = function (){
	  ...
	  this.upzero2=/^([1-9]+(\.[0-9]{2})?|0\.[1-9][0-9]|0\.0[1-9])$/;//大于0，两位小数
	  this.inputReg();
    }
   ...
   validate.prototype = {
	    ...
	    inputReg:function(){
            ...
            value.addEventListener("keyup", function(event){
			    ...
			    switch (value.getAttribute("reg")) {
			      case  "upzero2":
			      value.value = event.target.value.replace(/^([1-9]+(\.[0-9]{2})?|0\.[1-9][0-9]|0\.0[1-9])/g,'');  //大于0，两位小数
				  case  "number"  :
				  value.value = event.target.value.replace(/[^\d]/g,'');  // 如果不匹配数字 ，则清空输入的文字
				  //value.value = event.target.value.replace(/[^\d]/g,''); 
				  case "num-a-A"	:
				  value.value = event.target.value.replace(/[^0-9a-zA-Z]/g,'');  // 只能输入数字跟字母，则清空输入的文字 	
				 }
                    //if(reg){
